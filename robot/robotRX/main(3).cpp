...
/* Definir Modo Estatico ou Dinamico */
/* Modo Estatico  1 */
/* Modo Dinamico  0 */
#define ONLY_XBEE   0 //Para debug pela porta serie por 1 
/* --------------------------------- */
        ...
        switch(status){
            ...
            case WAIT_FOR_DATA:
                ...
                if( readContiki_frame()==1 ){
                    if( messageIn[PAYLOAD_CONTIKI_MSG_POS] == '*' && messageIn[PAYLOAD_CONTIKI_MSG_POS+1] == 0x00){ //Ignorar ACK
                        break;
                    }
            ...
            case FINISH_TRANSFER_DATA_TO_CONTIKI_SINK:
                ...
                if( readContiki_frame()==1 ){
                    if( messageIn[PAYLOAD_CONTIKI_MSG_POS] == '#' && messageIn[PAYLOAD_CONTIKI_MSG_POS+1] == '*' ){
                        log_serial.printf("\r\nACK_2 recebido DADOS ENTREGUES (Vou Voltar): \r\n");
